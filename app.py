...
# Lets a user write (or generate) arbitrary Cypher and run it directly
# against the *same* Neo4j business graph that kg/graph_ingest.py built
# (Claimant / Policy / Claim / Payment / Adjuster + their relationships —
# see kg/build_graph.py for the schema-metadata graph and
# kg/graph_ingest.py for this business-data graph). The query result is
# rendered as a real interactive node/edge graph, not a static diagram.
...
_GV_DOMAIN_COLORS = {
    ...
    "Adjuster": ("#fbbf24", "#d97706"),
}
_GV_DEFAULT_COLOR = ("#cbd5e1", "#94a3b8")

_GV_MUTATING_KEYWORDS = re.compile(
    r"\b(CREATE|MERGE|DELETE|DETACH\s+DELETE|SET|REMOVE|DROP|CALL\s+db\.|"
    r"CALL\s+apoc\.[a-zA-Z.]*(create|merge|delete|remove))\b",
    re.IGNORECASE,
)

_GV_SAMPLE_CYPHER = {
    "Whole graph (sample)": "MATCH (n)-[r]->(m) RETURN n, r, m LIMIT 150",
    "Claimants → Claims → Policies": (
        "MATCH p = (c:Claimant)-[:FILED]->(cl:Claim)-[:COVERED_BY]->(pol:Policy) "
        "RETURN p LIMIT 75"
    ),
    "Claims with payments": (
        "MATCH p = (cl:Claim)-[:HAS_PAYMENT]->(pm:Payment) RETURN p LIMIT 75"
    ),
    "Claims sharing an adjuster": (
        "MATCH p = (c1:Claim)-[:HANDLED_BY]->(a:Adjuster)<-[:HANDLED_BY]-(c2:Claim) "
        "WHERE c1.claim_id < c2.claim_id RETURN p LIMIT 75"
    ),
    "Policies sharing an agent": (
        "MATCH p = (p1:Policy)-[:SAME_AGENT_AS]->(p2:Policy) RETURN p LIMIT 75"
    ),
}


def _gv_sanitize_value(value):
    """
    Recursively convert neo4j graph types (Node, Relationship, Path — and
    anything nested inside lists/dicts) into plain, Arrow-serializable
    Python values, so st.dataframe()/pyarrow don't choke on mixed
    struct/non-struct cells (which happens when a RETURN clause hands back
    nodes, relationships, or paths instead of scalars).
    """
    from neo4j.graph import Node, Relationship, Path

    if isinstance(value, Node):
        return {"_labels": list(value.labels), **dict(value)}
    if isinstance(value, Relationship):
        return {
            "_type": value.type,
            "_start": _gv_node_caption(value.start_node),
            "_end": _gv_node_caption(value.end_node),
            **dict(value),
        }
    if isinstance(value, Path):
        # Flatten to an ordered [start_node, rel, node, rel, node, ...] list,
        # each element already sanitized to a plain dict/string.
        flat = [value.start_node]
        for rel in value.relationships:
            flat.append(rel)
            flat.append(rel.end_node)
        return [_gv_sanitize_value(v) for v in flat]
    if isinstance(value, dict):
        return {k: _gv_sanitize_value(v) for k, v in value.items()}
    if isinstance(value, (list, tuple)):
        return [_gv_sanitize_value(v) for v in value]
    return value


def _gv_run_cypher(pipeline, cypher: str):
    """
    Execute arbitrary Cypher directly against Neo4j using the app's own
    connection settings (pipeline.cfg — the same NEO4J_URI / database as
    kg/graph_ingest.py), and return both a tabular DataFrame of the raw
    result and a neo4j.graph.Graph of every node/relationship the query
    actually touched (so we can draw the real graph, not a re-derived one).
    """
    ...
    try:
        with driver.session(database=cfg.neo4j_database) as session:
            ...
            records = list(result)          # fully consume before .graph()
            graph = result.graph()          # nodes/relationships touched
            rows = [
                {k: _gv_sanitize_value(v) for k, v in r.data().items()}
                for r in records
            ]
            # Stringify any remaining nested list/dict cells so every column
            # is a single, uniform (Arrow-friendly) type — dataframes with
            # struct-valued cells mixed with scalars fail to convert.
...
def _gv_node_caption(node) -> str:
    props = dict(node)
    for key in ("name", "policy_number", "claim_id", "payment_id",
                "adjuster_id", "insured_name", "payee_name", "claimant_id"):
        if props.get(key):
            return str(props[key])
    labels = list(node.labels)
    return labels[0] if labels else str(node.element_id)


def _gv_render_graph(graph, height: int = 620) -> None:
    """Render the actual nodes/relationships returned by a Cypher query as
    an interactive vis.js network, color-coded to match the domain schema
    used everywhere else in the app (see _render_kg_traversal_graph)."""
    ...
    for node in graph.nodes:
        labels = list(node.labels)
        bg, border = next(
            (_GV_DOMAIN_COLORS[l] for l in labels if l in _GV_DOMAIN_COLORS),
            _GV_DEFAULT_COLOR,
        )
        props = dict(node)
        tooltip_rows = "".join(
            f"<div><b>{k}:</b> {v}</div>" for k, v in list(props.items())[:14]
        )
        vis_nodes.append({
            "id": node.element_id,
            "label": _gv_node_caption(node),
            "title": (
                f"<div style='font-family:Inter,system-ui,sans-serif;font-size:11px;"
                f"max-width:260px'><b>:{':'.join(labels) if labels else '?'}</b>"
                f"<hr style='margin:4px 0;border-color:#e2e8f0'/>{tooltip_rows}</div>"
            ),
            "shape": "dot",
            "size": 20,
            "color": {"background": bg, "border": border,
                      "highlight": {"background": bg, "border": border}},
            "font": {"size": 12, "color": "#1e293b", "face": "Inter,system-ui,sans-serif"},
            "borderWidth": 2,
        })
    ...
    for rel in graph.relationships:
        vis_edges.append({
            ...
            "color": {"color": "#7c3aed", "opacity": 0.75},
            "font": {"size": 9, "color": "#7c3aed", "strokeWidth": 2, "strokeColor": "#f8fafc"},
            "width": 1.5,
            "smooth": {"type": "dynamic"},
        })
    ...
    n_labels = sorted({lbl for n in graph.nodes for lbl in n.labels})
    legend_html = "".join(
        f"<span style='display:inline-flex;align-items:center;gap:5px;margin-right:14px'>"
        f"<span style='width:12px;height:12px;border-radius:50%;"
        f"background:{_GV_DOMAIN_COLORS.get(lbl, _GV_DEFAULT_COLOR)[0]};"
        f"border:2px solid {_GV_DOMAIN_COLORS.get(lbl, _GV_DEFAULT_COLOR)[1]};"
        f"display:inline-block;flex-shrink:0'></span>"
        f"<span style='color:#1e293b;font-weight:600;font-size:11px'>:{lbl}</span></span>"
        for lbl in n_labels
    )
...
<style>
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  #graph {{ width:100%; height:{height - 40}px; background:#f8fafc; }}
  div.vis-tooltip {{ background:transparent !important; border:none !important; box-shadow:none !important; padding:0 !important; }}
  #footer {{
    padding:6px 12px; font-size:10.5px; color:#475569; display:flex;
    gap:10px; align-items:center; flex-wrap:wrap; justify-content:space-between;
    border-top:1px solid #e2e8f0; background:rgba(248,250,252,0.97);
  }}
</style></head>
<body>
<div id="graph"></div>
<div id="footer">
  <span><b style="color:#1e293b">{len(vis_nodes)}</b> nodes &nbsp;·&nbsp; <b style="color:#1e293b">{len(vis_edges)}</b> relationships</span>
  <span style="display:flex;gap:10px;flex-wrap:wrap">{legend_html}</span>
</div>
<script>
(function() {{
  ...
  var options = {{
    physics: {{
      ...
      forceAtlas2Based: {{ gravitationalConstant: -70, springLength: 150, springConstant: 0.06, avoidOverlap: 0.8 }},
      stabilization: {{ enabled: true, iterations: 250 }},
    }},
    interaction: {{ hover: true, tooltipDelay: 60, dragNodes: true, dragView: true, zoomView: true }},
    edges: {{ selectionWidth: 3, hoverWidth: 2, smooth: {{ type: 'dynamic' }} }},
...
def _render_graph_visualization_page(pipeline) -> None:
    """Full 'Graph Visualization' tab — write or generate Cypher and see it
    rendered against the exact business graph built by kg/graph_ingest.py
    (the same Neo4j instance/database the Cypher lane queries)."""
    st.markdown(f"""
...
""", unsafe_allow_html=True)
    ...
    if "gv_cypher" not in st.session_state:
        st.session_state.gv_cypher = _GV_SAMPLE_CYPHER["Whole graph (sample)"]

    gen_col, run_col = st.columns([3, 1])
    with gen_col:
        nl_question = st.text_input(
            "Generate Cypher from a question (optional)",
            placeholder="e.g. show claims connected through the same adjuster",
            key="gv_nl_question",
        )
    with run_col:
        st.write("")
        st.write("")
        generate_clicked = st.button("Generate Cypher", use_container_width=True)

    if generate_clicked and nl_question.strip():
        with st.spinner("Generating Cypher..."):
            try:
                st.session_state.gv_cypher = pipeline.cypher_gen.generate(nl_question)
            except Exception as e:
                st.error(f"Could not generate Cypher: {e}")

    st.text_area(
        "Cypher query",
        key="gv_cypher",
        height=140,
        help="Include the nodes/relationships/paths you want drawn in the RETURN "
             "clause, e.g. `RETURN n, r, m` or `RETURN p` for a path.",
    )

    run_clicked = st.button("▶ Run Cypher", type="primary")

    cypher_text = st.session_state.gv_cypher

    if run_clicked:
        if not cypher_text.strip():
            st.warning("Enter a Cypher query first.")
        elif _GV_MUTATING_KEYWORDS.search(cypher_text):
            st.session_state["gv_pending_mutation"] = cypher_text
            st.warning(
                "This query contains a write clause (CREATE / MERGE / DELETE / SET / "
                "REMOVE / DROP / etc.) and would modify the live graph that the rest of "
                "the app queries. Confirm below if that's really what you want."
            )
        else:
            st.session_state.pop("gv_pending_mutation", None)
            with st.spinner("Running Cypher against Neo4j..."):
                try:
                    df, graph = _gv_run_cypher(pipeline, cypher_text)
                    st.session_state["gv_last_result"] = (df, graph)
                    st.session_state["gv_last_error"] = None
                except Exception as e:
                    st.session_state["gv_last_result"] = None
                    st.session_state["gv_last_error"] = str(e)

    if st.session_state.get("gv_pending_mutation") == cypher_text:
        if st.button("⚠ Confirm and run write query anyway", type="secondary"):
            with st.spinner("Running Cypher against Neo4j..."):
                try:
                    df, graph = _gv_run_cypher(pipeline, cypher_text)
                    st.session_state["gv_last_result"] = (df, graph)
    ...
    if st.session_state.get("gv_last_error"):
        st.error(f"Cypher execution failed: {st.session_state['gv_last_error']}")

    result = st.session_state.get("gv_last_result")
    if result:
        df, graph = result
        _gv_render_graph(graph)
        with st.expander("Cypher executed", expanded=False):
...
with st.sidebar:
    ...
    if st.button("Query", key="nav_btn_query", use_container_width=True):
        st.session_state.nav_page = "Query"
        st.rerun()

    if st.button("Graph Visualization", key="nav_btn_graph", use_container_width=True):
        st.session_state.nav_page = "Graph Visualization"
        st.rerun()

    st.divider()

if st.session_state.get("nav_page") == "Graph Visualization":
    with st.sidebar:
        st.markdown("#### Quick Cypher")
        st.caption("Populates the editor — click ▶ Run Cypher to execute")
        for _label, _q in _GV_SAMPLE_CYPHER.items():
            if st.button(_label, key=f"gv_sq_{_label}", use_container_width=True):
                st.session_state["gv_cypher"] = _q
                st.rerun()
        st.divider()
...
# ── Page routing ─────────────────────────────────────────────────────────────
if st.session_state.get("nav_page") == "Graph Visualization":
    _render_graph_visualization_page(st.session_state.pipeline)
    st.stop()
